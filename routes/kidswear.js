...
router.get("/", async (req, res, next) => {
  ...
  try {
    ...
    let kidswear = await KidsWear.find(queryObject)
      ...
      .skip((pageNumber - 1) * productsLimit); //finding all the kidswear items according to the query
    ...
    if (!kidswear || kidswear.length === 0) {
      res.status(200).json({
        success: false,
        message: "No kidswear items Not found",
        kidswear: [],
      });
    }

    // console.log("Kidswear Items:", kidswear.length);
    let extraPages = Math.ceil(kidswear.length / productsLimit);
    if (kidswear.length > productsLimit) {
      kidswear = kidswear.slice(0, productsLimit); //slicing the kidswear items according to the limit
...
router.post("/", async (req, res, next) => {
  try {
    ...
    const newKidsWear = await KidsWear.create({
      title,
      description,
      image,
      affiliateLink,
